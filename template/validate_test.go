...
func TestValidate_Matrix(t *testing.T) {
	tests := []struct {
		...
		{
			name: "period default is an object",
			tmpl: tmpl(only(&template.Variable{
...
// TestParse_UnknownWrapperKeysTolerated documents the deliberate boundary:
// the strict unknown-field decode applies to the rendered body against its
// target request type, not to the document wrapper.
func TestParse_UnknownWrapperKeysTolerated(t *testing.T) {
	doc := `{"target":"request","author":"finance","body":` + validBody + `}`
	if _, err := template.Parse([]byte(doc)); err != nil {
		t.Errorf("Parse() = %v, want nil for an unknown wrapper key", err)
	}
}
