...
- `PULSE_RANGE_TABLES_DIR` — directory of JSON files auto-loaded as `RangeTables` (named labeled-date-range sets) at `pulse.New` time. Each `*.json` is a bare array of `{label,start,end}` objects (or a wrapped `{"description":...,"ranges":[...]}`); the filename minus `.json` becomes the registered table name. Registered ranges are validated via the shared range-compilation pass (`PULSE_RANGE_*` codes). Honoured when `pulse.Options{RangeTablesDir}` is empty; a name declared both programmatically and on disk is a hard error.
