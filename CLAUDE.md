...
| A registered field type | `skills/type-<kebab>.md` (atomic skill) + CLAUDE.md "Byte-layout invariants" + **`.claude/reference/byte-layout.md` (load it BEFORE the change)** + `skills/cohort-schema-design.md` | `TestOperatorHasAtomicSkill`, `TestAtomicSkillHasRequiredSections`, `TestSkillTokenBudget`, `TestSkillsCoverAllFieldTypes`, `TestClaudeMdMentionsFormatVersion` |
...
| Shard archive layout (entry names, `_schema.pulse` block, magic dispatch, dict prefix rule) | CLAUDE.md "Byte-layout invariants" (the `PK\x03\x04` + `_schema.pulse` literals must stay in that SECTION — `TestShardArchiveLayoutDocumented` slices by heading) + **`.claude/reference/byte-layout.md`** + `skills/cohort-schema-design.md` (Sharded) | `TestShardArchiveLayoutDocumented`, `TestSkillsCoverShardingTopics` |
| The sidecar point-lookup index (`cohort.pulse.<keyhash>.idx`, `IndexFormatVersion`, `service.classifyIndexFreshness`, the `Service.fingerprints` memo), the sidecar index MANIFEST (`cohort.pulse.indexes.json`, `encoding.IndexManifestSuffix`), the keyable-type policy (`processing.IsIndexKeyableFieldType`), or the projected buffered decode contract (`processing.NeededFields`, `Schema.BuildDecodePlan`, `Options.DisableProjection`) | **`.claude/reference/byte-layout.md` (load it BEFORE the change, not after)** + CLAUDE.md "Byte-layout invariants" pointer line + `skills/cohort-schema-design.md` (Sidecar index) + `skills/tool-lookup.md` + `docs/src/internals/extension-points.md` + `.claude/reference/execution-modes.md`. A new `PULSE_INDEX_*` / `PULSE_LOOKUP_*` code additionally moves `errors/codes.go` + `errors/fixup_metadata.go` + the manifest golden. `format_version` does NOT move — none of these is payload-reachable, and the manifest document carries its OWN `format_version` (`"1"`) | `TestCodesHaveFixups`, `TestManifestErrorCodesComplete`, `TestNeededFields_*`, `TestProjection_*`, `TestReadRecordProjected_*` |
...
| Any `synth/` surface — profile capture (`ProfileOptions` / `Profile` document sections), `SpecFromProfile` translation, `Spec` slots, generation (samplers, copula, model draw, the rule pass), structural rules (`Spec.Rules`), the `--suggest-rules` detectors, or the fidelity report | **`.claude/reference/synthetic-data.md` (load it BEFORE the change, not after)** + `skills/synthetic-data.md` + `skills/synth-models.md` + `skills/synth-structural-rules.md` + the matching `skills/op-synth-<kebab>.md` for a distribution + `docs/src/cli/profile-create.md` / `docs/src/cli/synth-from-profile.md` / `docs/src/cli/synth-from-schema.md`. `format_version` does NOT move (`synth.Profile` / `synth.Spec` live in `synth/`, not `types/`, so they are unreachable from `descriptor.BuildPayloadSchema`); a new `PULSE_SYNTH_*` error code still moves `errors/fixup_metadata.go` + the manifest golden | `TestSkillsCoverAllSynthDistributions`, `TestManifestDistributionsComplete`, `TestSpecFromProfile_PreModelsDocumentUnchanged`, `TestSpecFromProfile_PreDiscreteDocumentReconstructsAsNormal`, `TestProfileModels_RidesTheExistingScan`, `TestModelSelection_NoUserFacingOverride`, `TestLatentFor_EveryDistributionIsClassified`, `TestFieldTypePredicates_ClassifyEveryDeclarableType`, `TestEmitSpec_RoundTripsToByteIdenticalCohort`, `TestValidateRules_ExprEnvIsTheConstraintEnv`, `TestClaimTarget_OptionGranularity`, `TestCodesHaveFixups` |
