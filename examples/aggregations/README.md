...
Each request runs against the shared `all_types.pulse` cohort, which carries one column for every one of the 19 supported field types. Together they exercise the type-aware aggregation code paths end to end without needing per-example fixtures.
...
| `03_geo_centroid_bbox.json` | `AGG_GEO_CENTROID` (3D unit-sphere mean) and `AGG_GEO_BBOX` |
