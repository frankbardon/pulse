...
// TestRuleClaim_ThreeParticipantMatrixKeepsTheSurvivorsCorrelated is the
// POSITIVE half of the claim's effect on the residual correlator, and it
// needs three participants to exist at all.
//
// TestRuleClaim_ExcludedFieldLeavesTheResidualCorrelation above asserts
// the drop through the WARNING only, and it cannot do better: with one
// of TWO participants gone the matrix is a scalar and
// buildResidualCorrelator legitimately returns nil, so "the claim
// removed one participant" and "the claim broke the correlator" produce
// identical output. Three participants separate them — one field is
// claimed, the surviving two must still come out of generation
// correlated at the rho the spec asked for.
//
// The measurement is on the fields' own RESIDUALS, not their values: a
// `normal` target's composed draw reduces exactly to
// prediction + ResidualStd*z, so subtracting the (known, integer-exact)
// prediction recovers sigma*z and the Pearson of the two is the figure
// buildResidualCorrelator was handed. Measuring raw values instead would
// fold each model's own predictor variance into the denominator and
// report an attenuated number that no threshold could be set against.
func TestRuleClaim_ThreeParticipantMatrixKeepsTheSurvivorsCorrelated(t *testing.T) {
	const (
		rows       = 4000
		wantRho    = 0.7
		rhoEpsilon = 0.04
	...
	build := func(rules []synth.RuleSpec) *synth.Spec {
		spec := modelFidelitySpec(rows,
			synth.FieldModelSpec{
				Field: "spend", Intercept: 100, ResidualStd: 10, Min: -1000, Max: 1000,
				Predictors: []synth.ModelPredictorSpec{
					{Kind: "categorical_level", Field: "region", Level: "west", Coefficient: 30},
				},
			},
			synth.FieldModelSpec{
				Field: "tenure", Intercept: 30, ResidualStd: 4, Min: -1000, Max: 1000,
				Predictors: []synth.ModelPredictorSpec{
					{Kind: "categorical_level", Field: "plan", Level: "pro", Coefficient: 8},
				},
			},
			synth.FieldModelSpec{
				Field: "visits", Intercept: 20, ResidualStd: 5, Min: -1000, Max: 1000,
				Predictors: []synth.ModelPredictorSpec{
					{Kind: "categorical_level", Field: "region", Level: "north", Coefficient: 6},
				},
			},
		)
		...
		// tenure x visits is the pair under test; it is deliberately the
		// one pair whose two models share NO predictor field, so nothing
		// but the correlated residual draw can produce it.
		spec.ResidualCorrelations = []synth.CorrelationSpec{
			{A: "spend", B: "tenure", Correlation: 0.5},
			{A: "spend", B: "visits", Correlation: 0.5},
			{A: "tenure", B: "visits", Correlation: wantRho},
		}
		spec.Rules = rules
		return spec
	}

	// survivorRho recovers the two surviving models' residuals from
	// generated bytes and returns their Pearson correlation.
	survivorRho := func(t *testing.T, data []byte) float64 {
		t.Helper()
		region := readCategoricalField(t, data, "region")
		plan := readCategoricalField(t, data, "plan")
		tenure := readF64Field(t, data, "tenure")
		visits := readF64Field(t, data, "visits")
		xs := make([]float64, len(tenure))
		ys := make([]float64, len(visits))
		for i := range tenure {
			pt := 30.0
			if plan[i] == "pro" {
				pt += 8
		...
		return testPearson(xs, ys)
	}

	free, _, err := synth.SynthBytes(build(nil), synth.Options{Seed: 7})
	if err != nil {
		t.Fatalf("rule-free SynthBytes: %v", err)
	}
	if got := survivorRho(t, free); math.Abs(got-wantRho) > rhoEpsilon {
		t.Fatalf("fixture broken before the claim: tenure x visits residual rho = %.4f, want %.2f+-%.2f",
			got, wantRho, rhoEpsilon)
	}

	claimed, res, err := synth.SynthBytes(
		build([]synth.RuleSpec{{SetExpr: map[string]string{"spend": "42"}}}),
		synth.Options{Seed: 7})
	if err != nil {
		t.Fatalf("claimed SynthBytes: %v", err)
	}
	if !hasWarning(res.Warnings, claimWarning("spend", 0, "set_expr", "linear model")) {
		t.Fatalf("the rule did not claim `spend`: %v", res.Warnings)
	}
	// The claim landed on the wire, so the remaining assertion is about
	// a matrix that really did lose a participant.
	for i, v := range readF64Field(t, claimed, "spend") {
		if math.Abs(v-42) > ruleValueTolerance {
			t.Fatalf("row %d: spend = %v, want the rule's 42", i, v)
		}
	}
	if got := survivorRho(t, claimed); math.Abs(got-wantRho) > rhoEpsilon {
		t.Fatalf("after the claim removed `spend`, the surviving pair's residual rho = %.4f, want %.2f+-%.2f — "+
			"the claim did not merely drop one participant, it broke the correlator for the rest",
			got, wantRho, rhoEpsilon)
	}
}

// testPearson is the assertion-side correlation. It is deliberately its
// own arithmetic rather than a reach into the package's internal
// pearson(): a test that measures with the function under test cannot
// fail when that function is wrong.
