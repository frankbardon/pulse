...
type ProfileOptions struct {
	...
	// SuggestRules enables the structural-rule DETECTION pass
	// (`profile create --suggest-rules <path>`): for every
	// low-cardinality field and every other field, measure
	// P(target null | field = level) off the same single scan and
	// propose a rule for each field whose levels split a target's null
	// rate into ~1 and ~0.
	//
	// Off by default, and ABSENT THE FLAG the profile document is
	// byte-identical — detection is a proposal pass, not a capture.
	//
	// WITH the flag the document gains no SECTION: the candidates reach
	// the caller on Profile.RuleCandidates, which is `json:"-"`, because
	// an unreviewed structural claim must not ride inside the document
	// that drives generation. The one thing that does move is
	// `warnings`, which gains the detector's own lines (what it could
	// not consider, what it counted rather than proposed, which
	// candidates rest on thin levels) — deliberately, so those survive
	// in a document rather than only on a terminal, which is how this
	// package's last three silent defects survived. Nothing here is read
	// back by SpecFromProfile and no generated byte moves;
	// TestSuggestRules_DocumentMovesOnlyItsWarnings pins exactly that
	// boundary. The candidates are written as a standalone rules file by
	// the CLI (see WriteRuleCandidates) for the analyst to edit and feed
...
type Profile struct {
	...
	// `synth from-profile --rules` consumes. It is `json:"-"` ON
	// PURPOSE: these are a PROPOSAL for a human to read, edit and feed
	// back, not a captured statistic, and writing them into the profile
	// document would put an unreviewed structural claim inside the
	// document that drives generation. It matches FieldModel.Residuals'
	// precedent: a capture product whose only consumer runs in the same
	// process, absent from a document read back from disk.
	//
	// The CLI writes them to the `--suggest-rules` path via
	// WriteRuleCandidates.
	RuleCandidates []RuleSpec `json:"-"`
}
...
func profileRecords(schema *encoding.Schema, r io.Reader, opts ProfileOptions) (*Profile, error) {
	...
	// gates is nil unless ProfileOptions.SuggestRules asked for
	// structural-rule detection AND the cohort has a low-cardinality
	// field to be a gate, so the per-row cost below is exactly zero for
	...
	// Rule-candidate detection runs after the marginals for the same
