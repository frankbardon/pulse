...
func profileCreateCmd() *cli.Command {
	return &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{Name: "suggest-rules", Usage: "Detect structural rules on the same scan — gating relationships (`set_null`) and co-missing question blocks (`null_together`) — and write them to this path as a standalone rules file (a bare JSON array, the shape `synth from-profile --rules` consumes). PROPOSED, never applied: read the `_evidence` on each candidate, correct it, delete what you do not believe. Near-miss blocks and always-null columns are reported in the warnings rather than proposed. The profile document itself is unchanged"},
