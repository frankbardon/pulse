...
func TestArrow_NativeDecimalRoundTrip(t *testing.T) {
	...
	d2, _, _ := encoding.ParseDecimal128("-0.000001")
	if err := w.WriteRow([]any{
		d2,
		encoding.PointF64{Lat: 0, Lon: 0},
		encoding.H3Cell(0x89283082807ffff),
	}); err != nil {
		t.Fatal(err)
	}
	...
	if got.Fields[0].Type != encoding.FieldTypeDecimal128 {
		t.Errorf("amount type = %s", got.Fields[0].Type)
	}
	...
	if got.Fields[1].Type != encoding.FieldTypePointF64 {
		t.Errorf("loc type = %s", got.Fields[1].Type)
	}
	if got.Fields[2].Type != encoding.FieldTypeH3Cell {
		t.Errorf("cell type = %s", got.Fields[2].Type)
	}
	if got.Fields[2].H3Resolution != 9 {
		t.Errorf("cell h3 resolution lost: %d", got.Fields[2].H3Resolution)
	}
	...
	if rows[0][0] != "123.456789" {
		t.Errorf("row0 amount = %q, want 123.456789", rows[0][0])
	}
	if rows[0][1] != "POINT(-122.418 37.775)" {
		t.Errorf("row0 loc = %q", rows[0][1])
	}
}
