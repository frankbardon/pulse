...
The usual cause is the pre-rounding gotcha above, so the message says so. Measured on the motivating profile at 20,000 rows: `familiarity <= 1` fires on 1,830 rows raw and 3,693 behind `{"set_expr": {"familiarity": "int(familiarity)"}}` — both silent; only the empty case warns. Counting consumes no RNG and allocates nothing per row: generated bytes are identical with it and without it.
