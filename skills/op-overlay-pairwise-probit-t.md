...
Intra-matrix pairwise along one axis of the SAME crosstab: `row` scope pairs row indices per column, `column` pairs column indices per row. Sibling of `OVERLAY_PAIRWISE_PROP_Z` — same proportion + n inputs, probit transform instead. Overlays decorate the host; they emit no `Response.Components` (this family READS them).
