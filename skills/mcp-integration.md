...
| `pulse_process_chain` | Execute a source-rooted linear chain of mergeable stages (stage N+1 feeds off stage N's rows). Per-stage overlays ride each stage's `Request.Overlays`; whole-chain overlays ride `ChainRequest.Overlays []*ChainOverlaySpec` (dual-slot design, E6-S2 types-only landing). | `request` (JSON-encoded `pulse.ChainRequest`) |
