...
- Whole-chain handler dispatch lands in E6-S3 (chassis), E6-S4 / E6-S5 (per-kind handlers). Predict-time validation lands in `descriptor/chain_overlay.go` (E6-S7) — `ValidateChain` walks `ChainRequest.Overlays` after the per-stage gate and emits `PULSE_OVERLAY_KIND_UNKNOWN` (anything outside `OVERLAY_INDEX_VS_STAGE` / `OVERLAY_DELTA_VS_STAGE` is rejected), `PULSE_OVERLAY_REFERENCE_UNKNOWN` / `PULSE_OVERLAY_TARGET_UNKNOWN` (StageRef resolution failures — same Index / Name / XOR / latest-stage-default contract the runtime `resolveChainStageRef` uses in `processing/overlay_chain_dispatch.go`), and `PULSE_OVERLAY_CHAIN_STAGE_SHAPE_DIVERGENT` (Ref and Target stages produce different host shapes per `inferChainStageShape`: `req.Crosstab != nil ⇒ MATRIX`, `req.Aggregations + req.Groups ⇒ SERIES`, `req.Aggregations only ⇒ SCALAR`). Every rejected `(Ref, Target)` pair also lands in `ChainValidationResult.OverlaysSchemaDivergence` (PRD §I-FR-I3) so LLM planners can budget reshapes without re-parsing envelope details.
- E6-S11 widens the capability surface beyond the minimal allowed-kind hint shipped today.
