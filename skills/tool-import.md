...
- **`pulse_import` has no charset override.** A `.sav` that mislabels its encoding, or declares none and carries an 8-bit byte, fails `PULSE_SPSS_CHARSET_INVALID` / `_UNSUPPORTED` here with no recourse; the escape hatch is CLI-only (`pulse import spss --charset windows-1252`) or library-only (`spss.WithCharset`).
