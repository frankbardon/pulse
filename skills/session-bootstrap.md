...
Two additive `pulse synth from-profile` knobs (both absent reproduces the pre-flag output byte-for-byte) plus one on `pulse profile create`.
...
- `--suggest-rules <path>` (on `pulse profile create`) — DETECT structural rules on the same scan (no extra cohort read) and write them as a standalone rules file, the bare array `--rules` consumes unmodified: GATING relationships (`set_null`), CO-MISSING question blocks (`null_together`, admitted on identical null PATTERN — an identical null RATE is never enough, since two unrelated fields can share one) and EXACT DEPENDENCIES (`set_expr`, a field that IS a function of ONE other — bounded to `packed_bool`/`u4` targets and a `categorical_*`/`packed_bool`/`u4` source of at most 16 levels, with its band edges DISCOVERED from the data and its `null_together` inside the same rule). Near misses, almost-determined pairs, always-null and constant columns are reported in `warnings`, never proposed. Gating candidates are emitted first and dependency candidates LAST, because declaration order is applied order. PROPOSED, never applied: it finds the STATISTICAL gate and a human knows the SEMANTIC one — on the motivating cohort it proposes `aware` AND `familiarity == 1` with identical evidence to sixteen digits. Each candidate carries its measurement on `_evidence`, the one INERT slot of a rule (generation never reads it; an evidence-only rule is still `PULSE_SYNTH_RULE_EMPTY`). The profile document gains no section — absent the flag it is byte-identical, with it only `warnings` moves. Numeric gates are emitted through `round()`; bare `==` reads the pre-rounding float and under-fires. Detail: `synth-structural-rules`.
