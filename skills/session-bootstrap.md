...
- `--suggest-rules <path>` (on `pulse profile create`) — DETECT structural gating relationships on the same scan (no extra cohort read) and write them as a standalone rules file, the bare array `--rules` consumes unmodified. PROPOSED, never applied: it finds the STATISTICAL gate and a human knows the SEMANTIC one — on the motivating cohort it proposes `aware` AND `familiarity == 1` with identical evidence to sixteen digits. Each candidate carries its measurement on `_evidence`, the one INERT slot of a rule (generation never reads it; an evidence-only rule is still `PULSE_SYNTH_RULE_EMPTY`). The profile document gains no section — absent the flag it is byte-identical, with it only `warnings` moves. Numeric gates are emitted through `round()`; bare `==` reads the pre-rounding float and under-fires. Detail: `synth-structural-rules`.
