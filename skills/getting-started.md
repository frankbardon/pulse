...
| `pulse_process_chain` | Execute a source-rooted linear chain (`ChainRequest`) of mergeable stages — stage N+1's input cohort is stage N's output rows. Collapses N round-trips into one open + N stage validations. Mergeable-only (`processing.CanChainRequest`); non-mergeable stages return `PULSE_CHAIN_NOT_MERGEABLE`. Per-stage overlays ride `ChainRequest.Stages[i].Request.Overlays`; whole-chain overlays ride `ChainRequest.Overlays []*ChainOverlaySpec` (E6-S2 types-only landing; whole-chain handler dispatch lands in E6-S3+). | `request` (JSON-encoded `pulse.ChainRequest`) |
