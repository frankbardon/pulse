...
Compose-only parity overlay. Series-shape sibling of `OVERLAY_T_CELL`. Overlays decorate the host result; they do not emit `Response.Components`.
...
- **Byte-equal** to `TEST_WELCH` over the same inputs — shares `studentTTwoSidedP` + Welch-Satterthwaite df recurrence with `TEST_T` / `OVERLAY_T_CELL`.
- Missing reference row → `PULSE_OVERLAY_REF_ZERO` with `ref_missing=true`; entry NaN.
- Degenerate inputs (`se == 0`, `n < 2`) → NaN + same warning.
- Distinct from streamable SERIES arm of INDEX/DELTA_VS_REF — inferential family buffered by policy.
