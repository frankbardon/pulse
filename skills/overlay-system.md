...
| `OVERLAY_SHARE_OF_TOTAL` | `cell` | `matrix` | no (buffered) | `Margin` | Per-cell share-of-grand-total ratio `cell / grand_total`. CELL scope over a MATRIX (crosstab) host. The entire matrix sums to 1.0 in the absence of missing cells; renderers can present the layer as a single-population share projection. Structurally grand-axis-locked — the spec must populate `Ref.Margin`, but the runtime handler always reads the grand total regardless of the axis value. Completes the share triad (row / col / total). NOTE: the same kind name later routes to a streamable series-shape handler under Process context (E3); this row documents the MATRIX dispatch. |
