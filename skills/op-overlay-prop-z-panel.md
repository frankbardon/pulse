...
COMPOSE — MATRIX crosstab on every slot. Panel order `{Reference, Targets[0..N-1]}`. First kind to emit a per-cell vector payload (`MatrixCell.Value` as `[]float64`).

## Output

MATRIX — `Cells[r][c].Value` is `[]float64` of upper-triangular pairwise p-values (row-major, no diagonal), length `M(M-1)/2` for `M = N+1`. Pair index `i*(2*M-i-1)/2 + (j-i-1)`. `Baseline` unset.
