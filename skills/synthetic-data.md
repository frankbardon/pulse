...
**Reading a flag on a QUANTIZED target.** The discrete draw the earlier limitation asked for now exists (`bernoulli` / `discrete` marginals above) and the instrument covers it, so a flag here is a statement about the GENERATED COHORT rather than about the instrument's reach. Report-wide on the motivating cohort at 40,000 rows: 53 model entries, **53 comparable** (2 before FU-19), **40 flagged**; 2,499 predictor entries, **2,343 compared** (150 before), 97 flagged, 156 unestimable — 143 of those the top-K ranking mismatch above, 7 constant columns, 6 below the score's resolution. Measured `score_retention` across 2,193 calibrated terms: min 0.075, median 0.456, p75 0.814. Survey data is mostly binary and small-integer, so this is the common case rather than an edge.
