...
**Placement is the design.** The pass is `drawRow`'s last act, after the model stage. Running it earlier would let the model write the last word on its own target, so a `set_null` over a modelled field would be overwritten every row. **Accepted consequence: a field a rule NULLS still contributed its DRAWN value to any model using it as a PREDICTOR on that row.** The two-phase alternative was rejected — the propensity existed, the question was not asked. Write semantics: `set_null` sets the mask and LEAVES the drawn value in the row (same shape `nullableSampler` produces, so `isnull()` reads alike whatever the source; the wire gets the type's zero, never the masked value); `set` writes the value and CLEARS the mask, so declaration order composes symmetrically. A `set` literal is coerced to the row's Go shape once at compile time via `constantRowValue` — that is what turns a `set_*` target's `["tv","radio"]` into the row's `map[string]bool`.

**`set_expr` and `null_together` validate but are INERT** until their own stories; such a rule is not reached at all, `when` included, so an unimplemented slot cannot fail a working run.

Two silent gotchas. A `set_null` over a field not declared `"nullable": true` writes the type's zero as an ordinary value with NO null bit — the rule fires and the file cannot show it — so it warns naming the rule index and field. And `when` reads the row's PRE-ROUNDING float: `== k` over a numeric reconstructed as a continuous distribution fires only on draws landing exactly on `k` (for a clamped `normal`, the clamp's point mass), not on the whole wire-value-`k` bucket. Use comparisons (`>= 9`); `packed_bool` is exact (`1.0` / `0.0`). Rules reach GENERATED rows only — `synth from-profile --source` re-encodes the real partition straight through.
