...
- Models (`--fit-models`, additive/`omitempty` `models` section): one least-squares linear model per numeric field, that field regressed on every categorical **level** and `set_*` **option** in the cohort, fitted on the same single scan via `processing/regression`'s streaming OLS engine (per-field state is O(p²) and independent of row count; no second pass). One reference level per categorical is dropped and folded into the intercept — a full level set plus an intercept is rank-deficient (the dummy trap) — while set options are all kept, because a multi-select is not a partition. Also captures each model's residual scale and its row-aligned **fitted residuals** over a bounded reservoir (own RNG stream off `--seed`, so `--conditional`'s reservoir is never perturbed). A field with no usable predictors, too few non-null rows, or a rank-deficient design (nested `region`/`dma`, single-valued `wave`) loses only ITS model and is named in `Profile.Warnings` — never a refusal. Persisted as the additive/`omitempty` `models` section (E1-S3), one `{field, intercept, predictors, references, n_obs, r2, residual_std}` entry per fitted numeric; `predictors[]` is `{kind, field, level, coefficient}` addressed by **(field, level)** — the internal design-column name never reaches the document — and `references[]` names each categorical's DROPPED baseline level so the absent arm is a lookup, not a guess. Two slots deliberately do NOT serialise: the design-column name, and the row-aligned **residual reservoir** (`FieldModel.Residuals` / `ResidualPresent`), which is thousands of numbers per field whose only consumer runs in the same process as the capture — read them off `synth.Profile.FittedModels()`, never from a re-read document. A capture WITHOUT the flag emits no `models` key at all and is byte-identical to a pre-E1-S3 document; a capture WITH it moves nothing but that one key.
  **`models` RETIRES the numeric-target arms.** When the section is present `SpecFromProfile` populates `Spec.Models` and leaves `Spec.CategoricalNumericPairs` / `Spec.SetNumericPairs` EMPTY — one additive model already accounts for every predictor at once, whereas those arms each overwrite the same numeric and all but the first are dropped by `resolveConflicts` with a warning (thousands of them on a wide cohort). A model additionally PRE-CLAIMS its target in `resolveConflicts`, so a hand-authored spec that declares both loses the pair and is told which (`dropping categorical-numeric pair (...)`); a profile-derived spec never makes the claim in the first place. The decision is per-DOCUMENT, not per-field — a model dropped in translation (shape-fit target, unknown predictor, zero predictors) does NOT revive the arms; it warns `model for numeric field %q not applied: …`. The three NON-numeric-target arms (`CategoricalPairs`, `SetCategoricalPairs`, `SetSetPairs`) are untouched, so `--conditional` + `--fit-models` together is a supported combination that keeps both halves. Absent `models`, every arm populates exactly as before — the same silent-fallback shape `conditional.numeric_pairs`-over-`pairwise` already uses.
