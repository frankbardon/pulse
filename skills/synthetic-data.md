...
`synth.SpecFromProfile` reconstructs a Spec: numeric → `normal` clamped to observed min/max (or `mixture` when `shape` is present), categorical → `weighted_categorical`, date → `uniform_date`, `set_*` → `set_bernoulli`. Unsupported types → `PULSE_PROFILE_FIELD_UNSUPPORTED`; drop to schema mode for those.
