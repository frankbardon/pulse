...
Three writers reach a boolean and all three are covered. Its own sampler draws `bernoulli` directly. A conditional pair carries `Bernoulli: true` on `CategoricalNumericPairSpec` / `SetNumericPairSpec`, so each cell's captured `Mean` is a prevalence rather than a location (`Std` unused); without it the defect recurs once per cell. A model draws through `quantileFor`'s step `Q`, making it a **probit**: `P(1 | row) = Φ((μ − Φ⁻¹(1−p)) / σ)`. Coefficients order rows; they are not probability changes.
