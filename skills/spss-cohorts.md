...
**Still no CLI leaf and no export wiring** (`pulse export` / `newWriterForFormat` have no `.sav` arm; an SPSS output target is `PULSE_SPSS_EXPORT_UNSUPPORTED`) and no name validation.
