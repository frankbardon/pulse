...
`pulse export spss -i cohort.pulse -o out.sav` is the leaf; `pulse convert data.csv out.sav` reaches the same writer. Four flags, each one field of `spss.WriterOptions`: `--ignore-sidecar`, `--uncompressed`, `--charset`, `--sanitize-names`. Per-flag detail is in `session-bootstrap` and `docs/src/cli/export-spss.md`.
