...
Two things deliberately do NOT reproduce the source, because re-emitting them would be *wrong* rather than merely less faithful: byte order is always **little-endian** (`7/3` agrees) and `prod_name` identifies pulse. Records `7/21` and `7/22` carry each variable's **FINAL** name; ReadStat (haven, pyreadstat) refuses a file that spells the short name there. MOYR/QYR/WKYR keep both their raw seconds and their format code, so such a column still renders as a month or a quarter.
